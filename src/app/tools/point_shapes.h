...
public:
  ...
  void getModifiedArea(ToolLoop* loop, int x, int y,
                       gen::SymmetryMode symmetry, Rect& area) override {
    // Do nothing
  }
};
...
public:
  ...
  void transformPoint(ToolLoop* loop, const Stroke::Pt& pt) override {
    ...
    if (brush->type() == kImageBrushType &&
        (pt.symmetry == gen::SymmetryMode::RIGHT_DIAG ||
          pt.symmetry == gen::SymmetryMode::LEFT_DIAG ||
          pt.symmetry == gen::SymmetryMode::RIGHT_DIAG_REFLEX ||
          pt.symmetry == gen::SymmetryMode::LEFT_DIAG_REFLEX)) {
      x += brush->bounds().y;
      y += brush->bounds().x;
    }
    else {
  ...
  void getModifiedArea(ToolLoop* loop, int x, int y,
    ...
    if (symmetry == gen::SymmetryMode::RIGHT_DIAG ||
        symmetry == gen::SymmetryMode::LEFT_DIAG ||
        symmetry == gen::SymmetryMode::RIGHT_DIAG_REFLEX ||
        symmetry == gen::SymmetryMode::LEFT_DIAG_REFLEX)
...
private:
  CompressedImage& getCompressedImage(gen::SymmetryMode symmetryMode) {
    ...
    auto& compressPtr = m_compressedImages[comprImageIndex];
    if (!compressPtr) {
      compressPtr.reset(new CompressedImage(m_lastBrush->getSymmetryImage(comprImageIndex),
                                            m_lastBrush->getSymmetryMask(comprImageIndex),
                                            false));
    }
    return *compressPtr;
  }
